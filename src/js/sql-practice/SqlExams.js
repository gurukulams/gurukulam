...
class SqlExams {
    ...
    fetch('/api/exams/sql')
      ...
      .then(page => {
        ...
        if (error.name == "NoContent") {
          error.root.parent.innerHTML = '<p class="lead">There are no exams. But you can create one <a href="javascript://">here</a></p>';
          error.root.parent.querySelector("p > a").addEventListener('click', event => {
  ...
  createListElement(item) {
    ...
    liEl.innerHTML = '<div class="d-flex w-100 justify-content-between"><h5 class="mb-1">' + item.name + '</h5> <small>' + item.database + '</small> </div> <p class="mb-1">Some placeholder content in a paragraph.</p> <small data-id="' + item.id + '"><a href="#">Add Question</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="#">Edit</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="javascript://" class="del-btn">Delete</a></small>';
    return liEl;
  }
